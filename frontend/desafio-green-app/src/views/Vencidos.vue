<template>
  <div id="entrada-area">
    ...
    <div class="card" v-for="task in tasks" :key="task.id" @click="openModal(task)">
...
<div id="subtarefas-container" v-if="task.subtarefas.length > 0">
  <div id="subtarefas" v-for="sub in task.subtarefas" :key="sub.id">
    ...
    <div v-if="tasks.length === 0">Não há tarefas vencidas.</div>
  ...
  setup() {
    ...
    const fetchTasks = async () => {
      ...
      try {
        ...
        tasks.value = response.data.filter(task => {
          ...
          ...task,
          due_date: new Intl.DateTimeFormat('pt-BR').format(new Date(task.due_date)),
          subtarefas: Array.isArray(task.subtarefas) ? task.subtarefas : []
        }));

        console.log("Tarefas vencidas:", tasks.value);
      } catch (err) {
    ...
    const updateTask = async (id) => {
      try {
        ...
        if (updatedTask) {
          updatedTask.title = nomeTarefa.value;
          updatedTask.description = descricaoTarefa.value;
          updatedTask.due_date = dataVencimento.value;
        }
        closeUpdateModal();
      } catch (err) {
